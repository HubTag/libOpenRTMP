...
amf0_type_t amf0_next_type( byte* data, size_t data_len ){
    if( data_len < 1 ){
...
amf_err_t amf0_get_number( byte* data, size_t data_len, double *value ){
    size_t count = 0;
    AMF0_HARVEST_LENGTH(data, data_len, count, 9);
    AMF0_CHECK_TYPE( buffer, AMF0_TYPE_NUMBER, AMF_ERR_INVALID_DATA );
    byte flipped[8];
    ntoh_memcpy( flipped, buffer, 8 );
    *value = read_double_ieee( flipped );
    return 9;
}

amf_err_t amf0_get_boolean( byte* data, size_t data_len, int *value ){
    size_t count = 0;
...
amf_err_t amf0_get_string_length( byte* data, size_t data_len, size_t *value ){
    if( data_len < 3 ){
...
//String functions are used for normal and long strings, as well as XML documents
amf_err_t amf0_get_string( byte* data, size_t data_len, void *value, size_t value_len, size_t *continuation ){
    size_t len;
...
amf_err_t amf0_get_prop_length( byte* data, size_t data_len, size_t *value ){
...
amf_err_t amf0_get_prop_name( byte* data, size_t data_len, void *value, int value_len ){
    size_t count = 0;
    AMF0_HARVEST_LENGTH(data, data_len, count, 2);
...
//Dummy; do not use.
amf_err_t amf0_get_movieclip( byte* data, size_t data_len ){
...
amf_err_t amf0_get_null( byte* data, size_t data_len ){
    size_t count = 0;
    AMF0_HARVEST_LENGTH(data, data_len, count, 1);
...
amf_err_t amf0_get_undefined( byte* data, size_t data_len ){
    size_t count = 0;
    AMF0_HARVEST_LENGTH(data, data_len, count, 1);
...
amf_err_t amf0_get_reference( byte* data, size_t data_len, uint32_t *value){
    size_t count = 0;
...
amf_err_t amf0_get_ecma_array( byte* data, size_t data_len ){
...
amf_err_t amf0_get_object_end( byte* data, size_t data_len ){
    size_t count = 0;
    AMF0_HARVEST_LENGTH(data, data_len, count, 1);
...
//Unimplemented. Will implement if necessary.
amf_err_t amf0_get_strict_array( byte* data, size_t data_len ){
...
amf_err_t amf0_get_date( byte* data, size_t data_len, int* timezone, double* timestamp ){
    size_t count = 0;
...
//Dummy
amf_err_t amf0_get_unsupported( byte* data, size_t data_len ){
    emit_err("[Error] Trying to read an unsupported type from AMF!");
    return 0;
}

//Alias around amf0_get_string_length
amf_err_t amf0_get_long_string_length( byte* data, size_t data_len, size_t *value){
    return amf0_get_string_length(data, data_len, value);
}
//Alias around amf0_get_string
amf_err_t amf0_get_long_string( byte* data, size_t data_len, void *value, size_t value_len, size_t *continuation){
    return amf0_get_string(data, data_len, value, value_len, continuation);
}

//Alias around amf0_get_string_length
amf_err_t amf0_get_xmldocument_length( byte* data, size_t data_len, size_t *value){
    return amf0_get_string_length(data, data_len, value);
}
//Alias around amf0_get_string
amf_err_t amf0_get_xmldocument( byte* data, size_t data_len, void *value, size_t value_len, size_t *continuation){
    return amf0_get_string(data, data_len, value, value_len, continuation);
}

//Unimplemented. Will implement if necessary.
amf_err_t amf0_get_recordset( byte* data, size_t data_len ){
    emit_err("[Unimplemented] Trying to read Record Set from AMF!");
    return 0;
}
//Unimplemented. Will implement if necessary.
amf_err_t amf0_get_typed_object( byte* data, size_t data_len ){
    emit_err("[Unimplemented] Trying to read Typed Object from AMF!");
    return 0;
}
amf_err_t amf0_get_continue( byte* data, size_t data_len, void *value, size_t value_len, size_t *continuation ){
...
void amf0_print( byte* data, size_t data_len, rtmp_printer_t printer ){
    double num;
    int integer;
    uint32_t uinteger;
    char str[1000];
    ...
    while( r < data_len ){
        ...
        if( object_layer > 0 ){
            r += amf0_get_prop_name( data + r, data_len - r, str, 1000);
            printer->s("\"");
            printer->s(str);
            printer->s("\": ");
        }
        ...
        switch( type ){
            ...
            case AMF0_TYPE_STRING:
                r += amf0_get_string( data + r, data_len - r, str, 1000, nullptr);
                printer->s( "String: " );
                printer->s( str );
                break;
